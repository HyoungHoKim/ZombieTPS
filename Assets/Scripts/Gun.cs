...
public class Gun : MonoBehaviour
{
    ...
    private PlayerShooter gunHolder; // 총을 쏘는 주체
    ...
    // 탄 퍼짐
    [Range(0f, 10f)] public float maxSpread = 3f; // 총알이 퍼지는 최대 범위, 클수록 넓게 흩어짐
    [Range(1f, 10f)] public float stability = 1f; // 반동이 증가하는 속도, 높을 수록 반동이 증가하는 속도가 낮아져 안전성이 높아진다. 낮으면 낮을 수록 탄 퍼짐의 속도가 높아진다.
    [Range(0.01f, 3f)] public float restoreFromRecoilSpeed = 2f; // 연사를 멈춘 후 탄 퍼짐이 0이 되기까지 걸리는 시간
    private float currentSpread; // 현재 탄 퍼짐의 정도값
    private float currentSpreadVelocity; // 현재 탄 퍼짐의 반경의 실시간 변화량
    ...
    private void Shot(Vector3 startPoint, Vector3 direction)
    {
        ...
        if (Physics.Raycast(startPoint, direction, out hit, fireDistance, ~excludeTarget))
        {
            ...
            // 상대방으로부터 IDamageable 오브젝트를 가져오기 시도
            if (target != null)
            {
                ...
                // 상대방의 OnDamage함수를 실행시켜서 상대방에게 데미지 주기
    ...
    public bool Reload()
    {
        ...
        {
            // 이미 재장전 중이거나, 남은 총알이 없거나 탄창에 총알이 이미 가득한 경우 재장전 할 수 없다.
            return false;
        }
